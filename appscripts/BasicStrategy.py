...
class BasicStrategy (Strategy) :
    ...
    def __init__(s, pts, radii, knownPositions, gridHelper, blist, numtrials, bfollowers, reorderBuilders, rlist, optRs, modelRenderer, natt, nmod) :
        Strategy.__init__(s, pts, radii, knownPositions, gridHelper, blist, numtrials, bfollowers, rlist, optRs, modelRenderer, natt, nmod)
        if reorderBuilders :
            s.reorderBuilders()
        else :
            import cbutils
            cbutils.builderSanityCheck(blist, s.pts.size(), knownPositions)
    ...
    def reorderBuilders(s) :
        ...
        builderOrder, builderFallback, builderFallIndex = findBuilderOrder(s.builders, s.pts.size(), s.kp, writeDigraph='dotfile')
        ...
        for i in range(len(builderOrder)) :
            ...
            for bfi in range(len(s.bfollowers[bi])) :
                if builderOrder[i+bfi+1] != s.bfollowers[bi][bfi] :
                    print bfi, "Follower order is not obeyed by builder reordering : ", s.builders[bi].name(), ",", s.builders[builderOrder[i+bfi+1]].name()
                    assert None
                bfoll[i].append(i+bfi+1)
