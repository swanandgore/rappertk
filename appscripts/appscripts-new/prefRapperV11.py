...
from loopbuildV8 import Multiloop, incompleteSCcorrection2
   ...
   ################   COPY MAP AND MTZ FILES  #########################################
    ...
    for  i in range(nmodels):
        ...
        if mtzfn != None :
            ...
            parse_sfcheck_logfile("sfcheck.pre."+str(i)+".log","pre."+str(i)+"."+rtkmodel)
...
def parseLoopSequence(pdbfile,loopres,start,stop,chainid,mconly,pdbout) :
    ...
    print         rev_resnum.keys()
    
    for index in range(start,stop+1):
        for key in rev_resnum.keys():
            if str(index) in str(key) :
                sslen= sslen + 1 
    ...
    for k,v in nchids.items():
        if nchids[k] == chainid:
            chainkey.append(k)

    chainkey.sort()
    for k in chainkey:
        rev_resnum[int(nresnums[k])] = k
        
    for index in range(start,stop+1):
        if index in rev_resnum.keys():
            revindex = rev_resnum[index]
            badresids.append(nresids[revindex])
        else :
            missing = 1
