...
class XrayRanker :
    def __init__(s, phasedmtz, fplabel, fclabel, philabel, maptype, esmin, esmax) :
        s.mt = {"2F1-F2":0, "F1":1}
        s.phasedmtz, s.fplabel, s.fclabel, s.philabel, s.maptype, s.esmin, s.esmax = phasedmtz, fplabel, fclabel, philabel, s.mt[maptype], esmin, esmax
...
class XrayScorer :
    ...
    def gplot(s) :
        ...
        #cnsout, phasedmtz+"fo.map", phasedmtz+"fc.map", "FC", "PHIC", "F1", "mcsc")
...
if __name__ == "__main__" : callmain()
        ...
        #if f1label == "map" : xed = EDrestraint.makeEDrestraintFromMap(ptinds, "xcheckEDrestraint", mtzfn, 0.5, 5., 2.,0)
