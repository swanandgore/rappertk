...
def checkpart(lstart,lstop,chainid,res, resids, resnums, resns, chids, inscodes, pts, cadistCutoff=4.) :

    chains = [chainid]
    mcmiss, scmiss, chbr = set(), set(), set()
    print "Chains", chains,chids
    ...
    for chid in chains :
        ...
        for k in range(start,end+1) :
            ...
            if (int(resnums[k]) < int(resnums[lstart]) or int(resnums[k]) > int(resnums[lstop])):
                continue
            else:
                for an in resAtoms[resns[k]] :
                    if not res[k].has_key(an) :
...
def checkpartmiss(lstart,lstop,chainid,res, resids, resnums, resns, chids, inscodes, pts, cadistCutoff=4.) :

    chains = [chainid]
    mcmiss, scmiss, chbr = set(), set(), set()
    print "Chains",chainid , chains
    for chid in chains :
        keys = res.keys() ; keys.sort()
        start, end = None, keys[len(keys)-1]
