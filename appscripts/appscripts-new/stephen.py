...
cnsArgs[0]["wa"] = -1 ; cnsArgs[0]["num_cycles"] = 1 ; cnsArgs[0]["temperature"] = 5
...
def main() :
    ...
    if options.loopres != None : ## expecting resids in [RSNcRESIa] format, one per line
        ...
        for l in open(options.loopres, 'r').readlines() :
            lres = l[1:len(l)-2]
            if len(lres) > 5 : loopresids.append(lres)
    ...
    if re.compile("\.mtz$").search(options.sf) : shutil.copyfile(options.sf, "%s/rfree.mtz" % options.scratchdir)
    else : shutil.copyfile(options.sf, "%s/strfactors.mtz" % options.scratchdir)
    os.chdir(options.scratchdir)
    if not os.path.isfile("rfree.mtz") :
    ...
    from restraints import EDrestraint ; EDrestraint.setPenalty(1.) ; EDrestraint.setScatRad(1.) ;
    esmin, esmax, esmean, enrichment, xscoreCutoff = .000, 5., .0, 10, 0.9
    ...
    for cycle in range(startCycle, numRefCycles) :
        ...
        if not os.path.isfile(cnsout) :
            ...
            restoreChainids(cnsout, options.pdbfile)
        ...
        if cycle == 0 :
            badresids = None
            if options.loopres != None : badresids = options.loopres

        if cycle > 0 and len(badresids) < 1 and len(badscs) < 0 : print "DONE refinement" ; sys.exit(0)

        xrayRestGen = []
        if options.loopres and options.framework : xrayRestGen.append( prepareChain.CArestraintGenerator(options.loopres, 2., 5.) )
        ...
        ml.ranker.rankChildren = enrichment ; ml.ranker.rankRecurse = 1
