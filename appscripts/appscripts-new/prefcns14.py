...
import os, shutil, re
import geometry
from xray import cif2mtz, uniqueify, sfall, mtz2hkl2, cns_generate, cns_anneal, sgCCP4toCNS, fft, omitmap, mapman
...
def cnsRefinement(mtzin,pdbin, mtzout,pdbout, a,b,c,alpha,beta,gamma,sg,reso, cnsArgs,cycle, fp,sigfp,freer,extraTOPfile=None, extraPARfile=None) :
    mtz2hkl2(mtzin, "cns.hkl",fp,sigfp,freer)
    ...
    for cycle in range(startCycle, numRefCycles) :
        ...
        if not os.path.isfile(cnsout) :
            cnsRefinement("phased.mtz", modelIn, phasedmtz, cnsout,
                          ...
                          cnsArgs, cycle,options.f1label,options.sigf1label,options.freeRlabel)
