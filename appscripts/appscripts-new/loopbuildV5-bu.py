...
#    print loops, "\n", looptypes
    ...
    for i in range(len(loops)):
        if looptypes[i]  == 'loop':
            ...
            nrlooptypes.append(looptypes[i])
            print resids[loops[i][0]+1] , resids[loops[i][1]] , looptypes[i]

        elif looptypes[i]  == 'Nter':
            nrloops.append([loops[i][0],loops[i][1]-1])
            nrlooptypes.append(looptypes[i])
            print resids[loops[i][0]] , resids[loops[i][1]-1] , looptypes[i]            
...
class Multiloop :
    ...
    def run(s) :
        ...
        if debug == 1 : 
            ...
            for b in chainBreaks:
                print resids[b]
            for k in range(len(loops)):
                print resids[loops[k][0]] , resids[loops[k][1]] , looptypes[k]
            for k in range(len(loops)):
                print "Looprange",loops[k][1], loops[k][0] , looptypes[k]
                import sys ; sys.exit()
                for k in range(len(loops)):
                    print "LUS",loops[k][0],loops[k][1] , resids[loops[k][0]],resids[loops[k][1]]
                    import sys ; sys.exit()
        ...
        for k in range(len(loops)):
            skip = 0
            print "LOLO",loops[k],looptypes[k]

            if looptypes[k] == "NtoC" :
                if len(chainBreaks) > 0:
                    lchbr = []
                    for lbr in chainBreaks:
                        if lbr in range(loops[k][0],loops[k][1]+1) :
                            lchbr.append(lbr)
                ...
                if len(chainBreaks) > 0:
                    ...
                    if len(lchbr) == 0:
                        ...
                        if s.caRad != None : 
                            ...
                            for xx in range(lchbr[0]+1,loops[k][1]+1) : 
                                notbuilding.append(resnums[xx]);
                                print "Notbuikd",resnums[xx]
                    else:
                        for r in range(len(lchbr)):
...
################# End - - - - Add missing atoms ###################################################                
        ...
        for li in range(len(loops)) :
            ...
            #print "Startindex",resids[startindex] , resids[endindex], looptypes[li]
        ...
        for li in range(len(loops)) :
            ...
            startindex, endindex = loops[li]
            chid = chids[startindex]
#            print "Startindex",startindex , endindex, looptypes[li]
