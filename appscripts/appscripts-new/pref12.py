...
### took out test for pooronly == 0 when extractinbg sections.
   ...
   ################   COPY MAP AND MTZ FILES  #########################################
    ...
    if  mapfn != None:
        ...
        if poorOnly == 1 :
            ...
            if (chainid != None) or (start != None and stop != None and chainid != None ):
                #badresids = []
                badresids1 = list( set(badresids).intersection(bad) )
                print "getting intersection badresids ",badresids
                print "getting intersection bad ",bad
                print "getting intersection badresids1 ",badresids1
                ...
                badscs = list( set(badresids).intersection(badsides) )
                
                badresids = badresids1
                #import sys ; sys.exit()
    ...
    for  i in range(nmodels):
        ...
        if (  (badresids != None and  len(badresids) > 0  and poorOnly ==  1 ) or (poorOnly == 0 and badresids == None ) or (poorOnly == 0 and len(badresids) > 0) ) :
            ...
            ml.ranker = None 
            if poorOnly == 1 and start == None and stop == None and chainid == None  :
                ml.poor = "poor"
            if start != None and stop != None and chainid != None :
            ...
            if mtzfn != None  and useomitmap == 1 :
                ...
                print "Map fn is",mapfn,mapcoeff,ccp4map
                #import sys ; sys.exit()
        ...
        if opsax == 0 or mapfn == None or mconly == 1 or opsaxoff == 1:
            ...
            if len(badresids) == 0:
                ...
                if useomitmap == 1 and mtzfn != None : 
                    ...
                    nb2 = SCplacement("pre."+str(i)+"."+rtkmodel, scReduction, str(i)+"."+rtkmodel, "dotfile", useDEE, ccp4map, f1label, f2label , phiclabel, mapcoeff, esmin, esmax, None, useGivenRot, badresids, scPrepC).run()
