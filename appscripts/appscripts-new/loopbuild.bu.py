...
from prepareChain import removeSC, mergeBRlists, addNdummyGly, addCdummyGly, printResAtoms, PrepareChain, prepareRNAchain, incompleteSCcorrection , incompleteMCSCcorrection
...
def locateRegionsRandomize(resids, chids, badresids) :
    ...
    for oi in order :
        ...
        print "oi",oi,loops[oi] , looptypes[oi]


    return rloops, rlooptypes
...
class Multiloop :
    def __init__(s, pdbfile, badresids, mconly, caRad, scRad, scReduction, guidedSampling, popsize, backtrack, nmodels, outpdb, restrGen, prepC) :
    ...
    def run(s) :
        ...
        if s.mconly : res, pts = removeSC(res, pts, res.keys())
        ...
        if type(s.badresids) == type([1,2]) and len(s.badresids)==2 and type(s.badresids[0]) == type([1,2]) : # bands are given 
            ...
            if s.badresids == None :
                s.badresids = list( resids.values() )
            ...
            print "Before",len(pts)
            for k in range(len(loops)):
                for mc in mcmiss :
                    if mc in range(loops[k][0] ,  loops[k][1] ) : 
                        ## check which atoms are missing and then add
                        incompleteMCSCcorrection(res, resns, pts , mc) 

            print "After mc corre ",len(pts)
            for k in range(len(loops)):
                for sc in scmiss :
                    if sc in range(loops[k][0] ,  loops[k][1] ) : 
                        incompleteMCSCcorrection(res, resns, pts , sc) 

            print "After sc corre ",len(pts)
            ...
            ## hmmm 
            print "grr",loops , looptypes
            for  k in range(len(loops)) :
                print loops[k] , looptypes[k] 
                print resids[loops[k][0]] , resids[loops[k][1]]
                

        s.badresids = []
        for startindex,endindex in loops :
            for ri in range(startindex, endindex+1) : s.badresids.append( resids[ri] )

        scMissInds = None
        if not s.mconly :
            from prepareChain import buildAcb
            scMissInds = incompleteSCcorrection(res, resns, pts)
            for ri in scMissInds : buildAcb(res, resids, resnums, resns, chids, inscodes, pts, ri)
            vvpts = VecVecFloat(pts)
            for ri in scMissInds : s.prepC.makeChiBuilder(ri,ri,ri, res,resns,resids).buildSample(vvpts, 0)
        ...
        for li in range(len(loops)) :
            ...
            print startindex , chids[startindex] , endindex , chids[endindex]
            ...
            if looptypes[li] == "NtoC" :
                if resns[ loops[li][0] ] in [ "  A", "  T", "  C", "  G", "  U", ] :
                    ...
                    bl,nt,bf,rl,dum = s.prepC.preparePeptideChain(chid, res, resids, resnums, resns, chids, inscodes, pts, s.mconly, 1, guidedSamplingRadius, s.caRad, s.scRad, scMissInds)
            elif looptypes[li] == "Nter" :
                dum, firstindex = addNdummyGly(chid, res, resids, resnums, resns, chids, inscodes, pts)
                bl,bf,nt,rl = s.prepC.prepareChainTerminal("Nterm", startindex, endindex, firstindex, res, resids, resnums, resns, chids, inscodes, pts, s.mconly, s.caRad, s.scRad, guidedSamplingRadius, scMissInds)
            elif looptypes[li] == "Cter" :
                dum, lastindex = addCdummyGly(chid, res, resids, resnums, resns, chids, inscodes, pts)
                bl,bf,nt,rl = s.prepC.prepareChainTerminal("Cterm", startindex, endindex, lastindex, res, resids, resnums, resns, chids, inscodes, pts, s.mconly, s.caRad, s.scRad, guidedSamplingRadius, scMissInds)
            elif looptypes[li] == "loop" :
                if endindex-startindex < 5 and random.random() > 1.5 :
                    bl,bf,nt,rl = s.prepC.preparePeptideLoop(startindex, endindex, res, resids, resnums, resns, chids, inscodes, pts, s.mconly, s.caRad, s.scRad, scMissInds, guidedSamplingRadius)
                else :
                    bl,bf,nt,rl = s.prepC.preparePeptideMidloop(startindex, endindex, res, resids, resnums, resns, chids, inscodes, pts, s.mconly, s.caRad, s.scRad, scMissInds, guidedSamplingRadius)
                    reorderBuilders = None
            else : print "Dont understand looptype", looptypes[li] ; assert None
            mergeBRlists(blist, bfoll, numtrials, rlist, bl, bf, nt, rl, dummies, dum)
            bandInfrastructure.append( (bl,nt,bf,rl, reorderBuilders) )
        print "DUMMIES", dummies.keys()
        #sys.exit(0)
        ...
        if verbose(6) : printResAtoms(res,resids)
