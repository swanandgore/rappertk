...
class Multiloop :
    def __init__(s, pdbfile, badresids, mconly, caRad, scRad, scReduction, guidedSampling, popsize, backtrack, nmodels, outpdb, restrGen, prepC,sconly=None, cacaCutoff = 5.,null=[]) :
    ...
    def run(s) :
        ...
        for li in range(len(loops)) :
            ...
            if s.sconly == 1 :
                ...
                if((int(resnums[endindex])+1) not in rev_resnum.keys()):
                    print "Anchor N residue for N-terminal to be modelled needs to be present in the pdbfile", resids[endindex+1]
                    sys.exit()
        ...
        nbuilt = 0
        all_built = {};        all_unbuilt  = {}
        for nm in range(s.nmodels) :
            all_built[nm] = [] ;              all_unbuilt[nm] = []
            print "++++ Building model %d of %d models asked for. ++++" % (nm , s.nmodels)
            built, unbuilt = bandBuild(loops, looptypes, bandInfrastructure, res, resids, resnums, resns, chids, inscodes, pts, extraRestraints, extraRestrOpt,
