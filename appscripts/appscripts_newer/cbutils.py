...
def builderSanityCheck(blist, numpts, knownPositions, knownORbuilt=1) :
    srcBldr = [-999] * numpts
    ...
    if knownORbuilt==1 : ## enforce that all atoms are either known or in op of some builder
...
def findBuilderOrder(blist, bwt, numpts, knownPositions, knownORbuilt, writeDigraph=None) : # writeDigraph can be filename also
    builderSanityCheck(blist, numpts, knownPositions, knownORbuilt)
    ...
    topoOrder, topoParent, topoParentIndex = findTopologicalOrder(builderDep, bwt)
