...
def makePDBatomline2(atomname, atomnum, resname, resnum, chid, inscode, x, y, z,occ,bfac) :
    if atomnum > 9999 : atomnum = 9999
    if atomnum < -999 : atomnum = -999
    assert len(atomname) == 4
    assert len(resname) == 3
    assert len(chid) == 1
    assert len(inscode) == 1
    return "ATOM   %4d %4s %3s %1s%4s%1s   %8.3f%8.3f%8.3f%6.2f%6.2f" % (atomnum, atomname, resname, chid, resnum, inscode, x, y, z,occ,bfac)          
