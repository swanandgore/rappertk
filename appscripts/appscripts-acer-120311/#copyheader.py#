...
def main() :
    ...
    parser.add_option("--refpdb", action='store', type='string', dest='refpdbfile', help='starting pdb containing a model of pdb-ligand complex')
    ...
    removeMODEL(options.pdbfile)
    copyheader(options.pdbfile,options.refpdbfile)
...
def copyheader(pdbfile, refpdb) :
    ...
    for l in open(refpdb, 'r').readlines() :
        if  isPdbAtomLine(l) :
            ...
            remarklines.append(l)

    newlines = []
    for l in open(pdbfile, 'r').readlines() :
        newlines.append(l)
    op = open(pdbfile, 'w')
    for l in remarklines : op.write(l)
    for l in newlines : op.write(l)
    op.close()
