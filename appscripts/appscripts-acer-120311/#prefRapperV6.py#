...
def main() :
    ...
    parser.add_option("--dir-xyzin", action='store', type='string', dest='dir_pdbfile', help='Name of PDB input structure')
    ...
    parser.add_option("--dir-mapin", action='store', type='string', dest='dir_mapfn', help='Input map file',default=None)
    
    parser.add_option("--hklin", action='store', type='string', dest='mtzfn', help='Input phased MTZ file',default=None)
    parser.add_option("--dir-hklin", action='store', type='string', dest='dir_mtzfn', help='Input phased MTZ file',default=None)
    ...
    parser.add_option("--build-loop", action='store', type='int', dest='build_loop', help='Loop building mode', default=0)
...
###############################################################################################################################        
    ...
    if options.start == None and options.stop == None and options.loopres ==None and options.poorOnly == 0 :
	print "Rebuilding all chains"
	badresids = None
	if options.caRes == 0:
	    print "--use-ca-restraints to be set to 1" ; sys.exit()

    chainsFound  = getChains(options.pdbfile)
    print chainsFound
    if len(chainsFound) > 1 and options.chainid == None and options.start !=None :
        print "More than one chain detected , enter chain ID for loop"
        sys.exit()
    if len(chainsFound) == 1 and options.chainid == None and options.start !=None :
        options.chainid = chainsFound[0]

    if  options.chainid not in chainsFound and options.start !=None :
        print "Chain id not found in pdb file", options.chainid , chainsFound
        sys.exit()

    if options.start != None and options.stop !=None:
        prot = protein(modelIn, read_hydrogens=0, read_waters=0, read_hets=0)
        res, resids, resnums, resns, chids, inscodes, pts = prot2res.readProtRes(prot)
        loopseq = ""
        from data import three2one
        if (options.caRes != 0 and options.poorOnly == 0 and options.chainid !=None ):
            ...
            for loopcounter in range(options.start,options.stop+1):
                ...
                if aafound ==  0 :
                    ...
                    print "Ca positional restraints for this residue will no be added"
                    
        if (options.caRes == 0 and options.poorOnly == 0 and options.chainid !=None and  options.loopres == None ):
            if options.verbose > 7 :
                print "Reading loop seqeunce from coordinates file..."
                print "Loop to be modelled...",options.start,options.stop, options.chainid
            from data import three2one
...
###############################################################################################################################
    ...
    if  options.mapfn !=None:
        ...
        if (options.sg == None):
            ...
            for sg1 in options.sg:
                if sg1 in ["\n","\t","\s"]:
                    ...
                    ss = ss+sg1
            options.sg = ss
            print options.sg, len(options.sg)
        ...
        if (options.f2label == None or options.phiclabel == None ):
            ...
            sfall(modelIn, options.mtzfn, "phased.mtz",options.resolution,options.usefreer)
        ...
        if options.usefreer == 1 : 
            if options.freeRlabel == None :
                ...
                sys.exit()
            fftnew(options.mtzfn, ccp4map, modelIn,options.f1label,options.f2label,options.phiclabel,options.sigf1label,options.m,options.n,options.freeRlabel)
        else :
            fftnew(options.mtzfn, ccp4map, modelIn , options.f1label,options.f2label,options.phiclabel,options.sigf1label,options.m,options.n,None)
    ...
    if options.loopres !=None and options.caRes == 0 : 
        ...
        options.caRes = 0 ; options.caRad = None;	options.scRes = 0 ; options.scRad = None
    if options.scRes == 0 : print "No Calpha positional restraints will be used"; options.scRad = None
    if options.caRes == 0 : print "No sidechain centroid  positional restraints will be used"; options.caRad = None
...
#    ccp4map = "fft.2fofc.ext.map"
#    fcmap = "fft.fc.ext.map"
	...
	xscorer = XrayScorer(None, xscoreCutoff)
        ...
        if options.poorOnly == 1 :
            ...
            #badmcsc = xscorer.score(options.pdbfile, "phased.mtz") ; sys.exit()
	...
	#    for sc in scmiss :
        ...
        #print "After all ",len(pts)
        #modelRenderer.render(pts)
        #removeMODEL(modelIn);        adjustBfac2(modelIn, options.pdbfile,1, options.mcBfac, options.scBfac)
    ...
    if options.loopres != None and options.caRes == 0 :
	loopresids,opsaxoff,nullres = parseLoopSequence(modelIn,options.loopres,options.start,options.stop,options.chainid,options.mconly,pdboutpath)
        print nullres ;
        #sys.exit()
	badresids = loopresids ;
	...
	#prot = protein(modelIn, read_hydrogens=0, read_waters=0, read_hets=0)
        #res, resids, resnums, resns, chids, inscodes, pts = prot2res.readProtRes(prot)
	#mcmiss, scmiss, chainBreaks = checkProtChainsV2.checkanchors(options.start,options.stop,options.chainid,res, resids, resnums, resns, chids, inscodes, pts, options.cacaCutoff)

        #if len(mcmiss) > 0 :
	#    for mc in mcmiss :
        #        if mc != ' CA ':
        #            incompleteSCcorrection2(res, resns, pts , mc) 
        #if options.mconly != 1 and len(scmiss) > 1 :
	#    for sc in scmiss :
        #        incompleteSCcorrection2(res, resns, pts , sc) 
    ...
    if options.loopres and options.framework and options.caRes != 0 :
    ...
    for  i in range(options.nmodels):
        nb = 0 
        print nullres #; sys.exit()
        if (options.sconly != 1 and ( badresids == None or len(badresids) != 0 or options.poorOnly != 0)): 
            ml = Multiloop(modelIn, badresids, options.mconly, options.caRad, options.scRad, scvdwr, guidedsampling, popsize,
                           options.backtrack, 1 , "pre"+rtkmodel+"."+str(i), xrayRestGen, multiPrepC,options.sconly, options.cacaCutoff,nullres) # sconly=None, cacaCutoff = 5.,null=[]) :
        ...
        if options.opsax == 0 or options.mapfn==None or options.mconly == 1 or (opsaxoff == 1 and nb!=1):
            ...
            elif (options.start != None and options.stop != None and options.chainid != None and options.framework == 1):
                badresids = None

            elif (  options.poorOnly == 0 and options.start != None and options.stop != None and options.chainid != None):
                badresids = loopresids 
            ...
            if len(badresids) > 250:
                ...
                nb2 = SCplacement("pre"+rtkmodel+"."+str(i), options.scReduction, rtkmodel+"."+str(i) , "dotfile", useDEE, options.mapfn, options.f1label, options.f2label , options.phiclabel, mapcoeff, esmin, esmax, None, useGivenRot, badresids, scPrepC).run()
                if nb2 != 1:
        ...
        copyWater(options.pdbfile, rtkmodel+"."+str(i)) ## automatic water addition can go here
        removeMODEL(rtkmodel+"."+str(i)) ## automatic water addition can go here
                        ...
                        if lr in rev_resnum.keys() :
                            ...
                            for atom in resAtoms[AA13[aa]] : 
                                if atom in res[resindex].keys():
                                    ...
                                    print "ABT2ADD=",aname
                                    if atom == ' CA ' :
                                        null.append(resids[resindex])
                        else  : 
                            aa = loopres[lcounter+lr]
                            aname = AA13[aa]
                            for atom in resAtoms[AA13[aa]] : 
                                initatoms = initatoms + 1
                                print "Making...t0",atom,initatoms,aname, lr , chainid
                                nl = makePDBatomline2(atom,initatoms,aname,lr , chainid, ' ',0.000, 0.000, 0.000, 1.0,20.0) 
                                newfileList.append("%s\n"%nl)
                                print "ABT2ADD=",aname
                                id = makeResid(aname,chainid,lr,' ') 
                                if atom == ' CA ' :
                                    null.append(id)
                    ...
                    if lr in rev_resnum.keys() :
                        ...
                        for atom in resAtoms[AA13[aa]] : 
                            if atom in res[resindex].keys():
                                ...
                                print "Making..t1.",atom,initatoms,aname, lr , chainid
                                nl = makePDBatomline2(atom,initatoms,aname,lr , chainid, ' ', pts[res[resindex][atom]][0], pts[res[resindex][atom]][1], pts[res[resindex][atom]][2],1.00,bfactor )
                                newfileList.append("%s\n"%nl)
                                #if aname == ' CA ' :
                                #    null.append(resindex)
                            else :
                                
                                initatoms = initatoms + 1
                                print "Making..t2.",atom,initatoms,aname, lr , chainid
                                nl = makePDBatomline2(atom,initatoms,aname, lr, chainid, ' ',0.000, 0.000, 0.000,1.0,20.0) 
                                newfileList.append("%s\n"%nl)
                                id = makeResid(aname,chainid,lr,' ') 
                                if atom == ' CA ' :
                        ...
                        for atom in resAtoms[AA13[aa]] : 
                            initatoms = initatoms + 1
                            print "Making...t3",atom,initatoms,aname, lr , chainid
                            print resnumber, prevresnumber+1 , inloop , chainidref , chainid ,prevresnumber
                            nl = makePDBatomline2(atom,initatoms,aname,lr , chainid, ' ',0.000, 0.000, 0.000, 1.0,20.0) 
                            newfileList.append("%s\n"%nl)
                            id = makeResid(aname,chainid,lr,' ') 
                            if atom == ' CA ' :
