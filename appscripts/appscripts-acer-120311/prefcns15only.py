...
def cnsRefinement(mtzin,pdbin, mtzout,pdbout, a,b,c,alpha,beta,gamma,sg,reso, cnsArgs,cycle, usefreer,f1label,sigf1label,freeRlabel,extraTOPfile=None, extraPARfile=None) :
    ...
    cns_anneal2(a, b, c, alpha, beta, gamma, sgCCP4toCNS[sg], reso,
    ...
    sfall(pdbout, "rfree.mtz", mtzout, reso,usefreer,f1label,sigf1label,freeRlabel)
...
def main() :
    ...
    for cycle in range(startCycle, numRefCycles) :
        ...
        rtkmodel = "0.model%d.pdb" % (cycle+1) # rappertk model to be generated in this cycle
        if os.path.isfile(cnsout) and os.path.isfile(rtkmodel) and  os.path.isfile("cns%d.pdb"%(cycle+1)) :
            print  "phased%d.mtz2fofc.map"%cycle
            continue
        ...
        shutil.copyfile(cnsout,"0."+rtkmodel)
