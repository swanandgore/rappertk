...
def mtzdump(mtzfile,fplabel,sigfplabel,freerlabel=None,fclabel=None,phiclabel=None):
    ...
    inputs = []
    inputs.append("go")
    es, ol, el = execCmd2("mtzdump hklin %s | grep ' H K L '" % mtzfile, inputs)
    if es !=0 :
        return None

    if  re.compile(fplabel).search(ol[0]) == None :
        print "%s not found in mtz file %s"%(fplabel,mtzfile)
        print "exiting.."
        return None
    if  re.compile(sigfplabel).search(ol[0]) == None :
        print "%s not found in mtz file %s"%(sigfplabel,mtzfile)
        print "exiting.."
        return None
    if freerlabel != None:

        if  re.compile(freerlabel).search(ol[0]) == None :
            print "%s not found in mtz file %s"%(freerlabel,mtzfile)
            print "exiting.."
            return None
    if fclabel != None:
        if  re.compile(fclabel).search(ol[0]) == None :
            print "%s not found in mtz file %s"%(fclabel,mtzfile)
            print "exiting.."
            return None        
    if phiclabel != None:
        if  re.compile(phiclabel).search(ol[0]) == None :
...
def fftnew(phasedmtz, ccp4map, pdbfile,fplabel,fclabel,phiclabel,sigfplabel,scale1=2., scale2=1.,freerlabel=None,onlyFC=0) :
    rstats(phasedmtz,"scaled.mtz",fplabel,sigfplabel,fclabel,phiclabel,freerlabel)

    map = "pre."+ccp4map
    cmd, inputs = "fft hklin %s mapout %s" % ("scaled.mtz", map), []
    inputs.append("xyzlim asu")
    if onlyFC == 1:
        inputs.append("labin -")
        if freerlabel == None :
...
def sfall(pdb, rfreemtz, phasedmtz, reso=None,usefreer=1,fplabel="FP",sigfplabel="SIGFP",freerlabel = "FreeR_flag" ) :
...
def cns_anneal2(a,b,c,A,B,G,sg,reso, cnshkl, mtffn, pdbfn, parCNS=None, logfile=None, wa=None, num_cycles=2, temp=5000, harmCA=None) :
    ...
    print "CNS Annealing for %d cycles starting at %dK" % (num_cycles, temp)
    print sg, reso
    anneal_inp = "%s/appscripts/anneal2.inp" % os.environ["RTKROOT"]
    inputs = []
    segids = findSegids(pdbfn)
    for l in open(anneal_inp, 'r').readlines() :
        if re.compile("{===>} a=").search(l) : inputs.append( '{===>} a=%f;' % a )
