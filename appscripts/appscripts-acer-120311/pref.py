...
cnsArgs = {}
for cycle in range(20) : cnsArgs[cycle] = {} ; cnsArgs[cycle]["num_cycles"] = 2 ; cnsArgs[cycle]["temperature"] = 5000 ## DANGER was 5000
    ...
    for l in open(filename, 'r').readlines() :
        if isPdbAAline(l) :
            if line2atomname(l) in [" N  "," CA "," C  "," O  "] :
                l = changeBfactorOccupancy(l, bfacs[0], line2occu(l))
            else :
                print "low",l
                l = changeBfactorOccupancy(l, bfacs[1], line2occu(l))
        lines.append(l)
    fp = open(filename, 'w')
...
#    cns_anneal(a, b, c, alpha, beta, gamma, sgtable[sg][1], reso,
...
def main() :
    ...
    parser.add_option("--a", action='store', type='float', dest='a', help='cell dimension a',default= None)
    ...
    parser.add_option("--resolution", action='store', type='float', dest='resolution', help='resolution of the data',default=None)
    ...
    for cycle in range(startCycle, numRefCycles) :
        ...
        if cycle > 3 : 
            ...
            toRemove = "phased%d.mtz2fofc.map"%(cycle - 2)
            if os.path.exists(toRemove) :
                os.remove(toRemove)

            toRemove = "anneal_2fofc.map_%d"%(cycle - 2)    #        anneal_fc.map_9
            if os.path.exists(toRemove) :
                os.remove(toRemove)

            toRemove = "anneal_fc.map_%d"%(cycle - 2)    #        anneal_fc.map_9
            if os.path.exists(toRemove) :
                os.remove(toRemove)
            
            toRemove = "generate.mtf_%d"%(cycle - 2)    #        anneal_fc.map_9
            if os.path.exists(toRemove) :
                os.remove(toRemove)

            toRemove = "premodel%d.pdb"%(cycle - 2)    #        anneal_fc.map_9
            if os.path.exists(toRemove) :
                os.remove(toRemove)
        ...
        if options.loopres and options.framework :
            xrayRestGen.append( prepareChain.CArestraintGenerator(options.loopres, 1., 3.) )

        if cycle > 0 :
            xrayRestGen.append( prepareChain.XrayRestraintsGenerator(phasedmtz+"2fofc.map", "map", "FC", "PHIC", "2F1-F2", esmin, esmax, esmean, ["SCLbuilder","ChiBuilder","CBbuilder"], ) )
        if cycle == 0 :
            badresids = None #options.loopres


        print "GRR", options.loopres  , options.framework

        if options.loopres and not options.framework :
            if badresids == None :
                badresids = options.loopres
                print "Assigining bad resids to ",options.loopres
            else :
                badresids = list( set(badresids).intersection(options.loopres) )
            print "bads" , badresids


        if cycle > 0 and options.noRTK == 1 :
            shutil.copy(cnsout, rtkmodel) ; continue

        if os.path.isfile(rtkmodel) :
            print "Conti"
            continue
