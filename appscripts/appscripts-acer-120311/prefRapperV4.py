...
###############################################################################################################################
    ...
    if  options.mapfn !=None:
        ...
        if (os.path.isfile(mapfilepath)==False) :
            ...
            shutil.copyfile(mapfilepath, "%s/%s" % (options.dir_xyzout,options.mapfn))
        ...
        if (options.f2label == None or options.phiclabel == None ):
            ...
            sfall(modelIn, options.mtzfn, "phased.mtz",options.resolution,None)
        ...
        if options.usefreer == 1 : 
            if options.freeRlabel == None :
                ...
                sys.exit()
            fft(options.mtzfn, ccp4map, options.f1label,options.f2label,options.phiclabel,options.sigf1label,options.m,options.n,options.freeRlabel)
        else :
            fft(options.mtzfn, ccp4map, options.f1label,options.f2label,options.phiclabel,options.sigf1label,options.m,options.n,None)
            
        fcmap = "%sFC.map"%options.mtzfn

        fft(options.mtzfn, fcmap,  options.f1label,options.f2label,options.phiclabel,options.sigf1label,options.m,options.n,options.freeRlabel)
        options.mtzfn = None
    ...
    if options.loopres != None and options.caRes == 0 :
	loopresids,opsaxoff = parseLoopSequence(modelIn,options.loopres,options.start,options.stop,options.chainid,options.mconly,pdboutpath)
	badresids = loopresids ;
    ...
    for  i in range(options.nmodels):
        ...
        if options.opsax == 0 or options.mapfn==None or options.mconly == 1 or (opsaxoff == 1 and nb!=1):
            ...
            print "dddd Here",options.sconly , options.poorOnly
