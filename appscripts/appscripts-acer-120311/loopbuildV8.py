...
from prepareChainV4 import removeSC, mergeBRlists, addNdummyGly, addCdummyGly, printResAtoms, PrepareChain, prepareRNAchain, incompleteSCcorrection , incompleteMCSCcorrection  , incompleteMCcorrection , incompleteSCcorrection2
...
class Multiloop :
    def __init__(s, pdbfile, badresids, mconly, caRad, scRad, scReduction, guidedSampling, popsize, backtrack, nmodels, outpdb, restrGen, prepC,sconly=None, cacaCutoff = 5.,null=[],modelN2C=0,natt=5,missingpts = {},loopclosure = 1 ):
    ...
    def run(s) :
        ...
        if type(s.badresids) == type([1,2]) and len(s.badresids)==2 and type(s.badresids[0]) == type([1,2]) : # bands are given 
            ...
            loops, looptypes = locateRegionsRandomize(resids, chids, s.badresids,s.modelN2C,s.poor) # locate loops and order them randomly
        ...
        for k in range(len(loops)):
            ...
            if s.caRad != None :
                ...
                if looptypes[k] == "loop" and len(loops) == 1 and s.poor != "poor":
                    ...
                    if nfound != None and cfound != None :
                        looptypes[k] = "loop"
                    ...
                    if nfound == None and cfound == None :
                        print "Anchor residues: Nterm %d [%s] and Cterm %d [%s] not found in coordinates "%(nanchor,lchid,canchor,lchid)
        ...
        for kx in range(len(loops)):
            if loops[kx][1] - loops[kx][0] > 700 : # and looptypes[kx] == "NtoC":
...
#                if endindex-startindex < 5 or random.random() > 1.5 :
                if endindex-startindex <= 2  or random.random() > 1.5 :
