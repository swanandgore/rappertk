...
def main() :
    ...
    for cycle in range(startCycle, numRefCycles) :
        ...
        badresids = list ( set(badmcsc[0]+badmc[0]+badpept[0]) )
        print "Badresids",badresids

        if cycle > 0 and len(badresids) < 1 and len(badscs) < 0 : print "DONE refinement" ; sys.exit(0)
        xrayRestGen.append( prepareChain.XrayRestraintsGenerator(phasedmtz+"2fofc.map", "map", "FC", "PHIC", "2F1-F2", esmin, esmax, esmean, [], ) )
        ...
        nb = ml.run()
        badresids = list(  set(  findChangedSC(cnsout, "pre"+rtkmodel) + badscs[0] )  )
