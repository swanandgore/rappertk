            ...
            for phi in range(-180,180,40) :
                for psi in range(-180,180,40) :
                    ...
                    for ri in range(100000) :
                        ...
                        nrotPS = tmp
                    print "ppr",phi, psi, resn, maxRI, nrotPS
                    nrotPS += 1
...
class PrepareChain :
    ...
    def prepareSConly(s,startindex, endindex, res, resids, resnums, resns, chids, inscodes, pts, scRad, scMissInds=[], caMissInds=[]) :
        blist, rlist = [], []
    ...
    def preparePeptideChain(s, chid, res, resids, resnums, resns, chids, inscodes, pts, mconly, buildfwd, guidedSamplingRadius, caRad, scRad, scMissInds=None,caMissInds=[]) :
        assert mconly == 1 or mconly == None
        ...
        for index in keys :
            ...
            if chids[index] == chid and not loop_started :
                loop_started, startindex = 1, index
            if chids[index] != chid and loop_started :
                loop_started, endindex = None, lastindex
            lastindex = index    
        ...
        if buildfwd == 1 :
            ...
            if mconly != 1 :
                ...
                if rest1 and not startindex in scMissInds :
                    rlist.append(rest1)
#                else :
...
#                    import sys ; sys.exit()
    ...
    def makeNumtrials(s, blist) :
        ...
        return numtrials
    def preparePeptideMidloop1(s, startindex, endindex, res, resids, resnums, resns, chids, inscodes, pts, mconly, caRad, scRad, scMissInds=[], guidedRadius=None,caMissInds=[]) :
    ...
    def preparePeptideMidloop(s, startindex, endindex, res, resids, resnums, resns, chids, inscodes, pts, mconly, caRad, scRad, scMissInds=[], guidedRadius=None,caMissInds = []) :
        ...
        for index in range(startindex, endindex) :
            ...
            i, j, k = index-2, index-1, index
            print "ijk",i,j,k, resids[i] , resids[j], resids[k]
        ...
        if not mconly :
            for index in range(startindex-1, endindex+2) :
                print "IIII", index, scMissInds
                if index in scMissInds :
                    print "Missing",resids[index]
                    continue
                if resns[index] in ["ALA","GLY"] : continue
                IPs, mean, count = [], [0.,0.,0.], 0
                for k,v in res[index].items() :
    ...
    def preparePeptideLoop1(s, startindex, endindex, res, resids, resnums, resns, chids, inscodes, pts, mconly, caRad, scRad, scMissInds=[], guidedRadius=None, taperRestraints=None,caMissInds=[]) :
        print "In peptide loop";
    ...
    def preparePeptideLoop(s, startindex, endindex, res, resids, resnums, resns, chids, inscodes, pts, mconly, caRad, scRad, scMissInds=[], guidedRadius=None, taperRestraints=None,caMissInds = []) :
        if not mconly in [1,None] : print mconly ; assert None
        blist, bfoll = [], {}
        useSCfollowers = 1
...
def incompleteSCcorrection2(res, resns, pts , ind) :
    scMissInds = []
    
    #if not resAtoms.has_key(resns[ind]) : continue
    scmiss = None
    for an in resAtoms[resns[ind]] :
        if an in [' CA ',' C  ',' C  ',' O  ' ] : continue
        if an in res[ind].keys() : continue
        scmiss = 1
        res[ind][an] = len(pts)
        pts.append( (0.,0.,0.) )
...
def incompleteMCcorrection(res, resns, pts , ind) :
    scMissInds = []
...
def removeResid2(ipdb,opdb,lpdb,forIndices) :
    newpts = []
    from pdbr import protein
    import prot2res
    from peptidebuild import ModelRenderer

    prot = protein(ipdb, read_hydrogens=1, read_waters=1, read_hets=1)
    res, resids, resnums, resns, chids, inscodes, pts = prot2res.readProtRes(prot)

    lres, lresids, lresnums, lresns, lchids, linscodes, lpts = {}, {} , {}, {}, {}, {}, []
    rcount, pcount  = 0 , 0 
    for k , v in res.items():
        if k in forIndices : 
            lresids[rcount] =  resids[k]
            lresnums[rcount] =  resnums[k]

            lresns[rcount] =  resns[k]
            lchids[rcount] =  chids[k]
            
            linscodes[rcount] =  inscodes[k]
            lres[rcount] =  {}
    ...
    ModelRenderer(lres, lresns, lchids, lresnums, linscodes, [], lpdb).render(lpts)
    from copyheader import copyheader
    copyheader(lpdb,ipdb)
    ...
    ModelRenderer(res, resns, chids, resnums, inscodes, [], opdb).render(newpts)
    from copyheader import copyheader
    copyheader(opdb,ipdb)
