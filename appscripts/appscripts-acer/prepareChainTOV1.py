...
class PrepareChain :
    ...
    def makeChiBuilder(s, i, j, k, res, resns, resids, addsample=None) :
        ...
        #print "makechi",i,j,k,resids[j],res[j]
    ...
    def prepareSConly(s,startindex, endindex, res, resids, resnums, resns, chids, inscodes, pts, scRad, scMissInds=[]) :
        blist, rlist = [], []
...
def prepareAlphaHelix(helixtype, res, resids, resnums, resns, chids, inscodes, pts, caRad, start, end, bootstrap, mconly, informed) :
    f = PrepareChain("PRL") #31052011:Takashi:define f
    ...
    if start < end :
        for ind in range(start,end+1) :
            bgrp = f.makeBgrp(ind-2, ind-1, ind, pts, res, resns, resids, mconly, 1, informed)
            blistsize = len(blist)
            blist = blist + bgrp
        ...
        for ind in range(start,end-1,-1) : # bcz end is start and start is end
            ...
            bgrp = f.makeBgrp(ind, ind+1, ind+2, pts, res, resns, resids, mconly, 0, informed)
            blistsize = len(blist)
            blist = blist + bgrp
            if len(bgrp) > 1 and useSCfollowers : bfoll[blistsize] = range(blistsize+1,len(blist))
    ...
    numtrials = f.makeNumtrials(blist) #31052011:Takashi:Added f.
...
def prepareBetaSheet(res, resids, resnums, resns, chids, inscodes, pts, caRad, starts, ends, sheetBS, directions, corrInds, mconly, informed) :
        ...
        for si in range(len(step)) :
            ...
            if sheetBS[si] and not strandBootstrapped[si] :
                ...
                blist.append( NanchorBuilder( VecInt(IPs), VecInt(OPs), consts, "NanchorBuilder [%s][%s]"%(resids[b1],resids[b2]),
                    VecFloat(pts[res[b1][' CA ']]), caRad, VecFloat(pts[res[b2][' CA ']]), caRad ) )
	    #
	    #31052011:Takashi:Added f. to makeBgrp
	    #
            if directions[si] == "fwd" : bgrp = f.makeBgrp(step[si]-2, step[si]-1, step[si], pts, res, resns, resids, mconly, 1, informed)
            else : bgrp = f.makeBgrp(step[si], step[si]+1, step[si]+2, pts, res, resns, resids, mconly, 0, informed)
            blistsize = len(blist)
            blist = blist + bgrp
            if len(bgrp) > 1 and useSCfollowers : bfoll[blistsize] = range(blistsize+1,len(blist))
    builtAtomInds = []
    ...
    numtrials = f.makeNumtrials(blist)
    return blist, bfoll, numtrials, rlist
