    ...
    def run(s) :
        ...
        if type(s.badresids) == type([1,2]) and len(s.badresids)==2 and type(s.badresids[0]) == type([1,2]) : # bands are given 
            ...
            for k in range(len(loops)):
                for mc in mcmiss :
                    ...
                    if mc in range(loops[k][0] ,  loops[k][1] + 1) : 
                        ## check which atoms are missing and then add
                        incompleteMCcorrection(res, resns, pts , mc) 
        ...
        for li in range(len(loops)) :
            ...
            if s.sconly == 1 :
                ...
                bl,bf,nt,rl = s.prepC.prepareSConly(startindex, endindex,  res, resids, resnums, resns, chids, inscodes, pts, s.scRad, scMissInds)
