...
from loopbuild9 import Multiloop, incompleteSCcorrection2

import prepareChainV4
import prepareChain
...
def main(pdbfile,pdbout,dir_xyzout,mapfn,mtzfn,caRes,scRes,caRad,scRad,scReduction,popsize,verbose,backtrack,rotLib,nmodels,mconly,sconly,opsax,natt,cacaCutoff,a,b,c,alpha,beta,gamma,sg,resolution,f1label,sigf1label,f2label,phiclabel,usefreer,freeRlabel,n,poorOnly,poorThreshold,loopres,start,stop,chainid,modelN2C,nativeBfac,mcBfac,scBfac,minXSig,maxXSig,edOpt,allOpt,closure="True",addsc="False",userot="True",mapformat="ccp4"):    
    ...
    print "	--add-sidechains ".ljust(50), 
    print "%20s "%addsc

    print "	--use-given-rotamer".ljust(50), 
    print "%20s "%userot
    ...
    if addsc not in ["True", "False"]:
        print "--add-sidechains needs to be either True or False, Incorrect value input " ,addsc
        import sys ; sys.exit()        

    if userot not in ["True", "False"]:
        print "        --use-given-rotamer needs to be either True or False, Incorrect value input " ,userot
        import sys ; sys.exit()        
...
################## Checks for range of residues #######################################
   ...
   ################   COPY MAP AND MTZ FILES  #########################################
    ...
    if  mapfn != None:
        ...
        if mapformat == "omit": useomitmap = 1
        if mapformat == "ccp4": useccp4map = 1
    ...
    multiPrepC = prepareChainV4.PrepareChain(rotLib)
    ...
    if mtzfn != None and usecnsmap == 1:
        mapcoeff = "%dF1-%dF2"%(n, n-1)
        xrayRestGen.append( prepareChain.XrayRestraintsGenerator(fofcmap, "map", f2label, phiclabel, mapcoeff, esmin, esmax, esmean, [], ) )
    ...
    for  i in range(nmodels):
        nb = 0 
        if ( ( (badresids != None and  len(badresids) > 0  and poorOnly ==  1 ) or (poorOnly == 0 and badresids == None ) or (poorOnly == 0 and len(badresids) > 0) ) and sconly == 0 ):
        ...
        if opsax == 0 or mapfn == None or mconly == 1 or opsaxoff == 1:
            ...
            print "Renaming 1 ", "pre."+str(i)+"."+rtkmodel, " to ",str(i)+"."+rtkmodel 

        elif  badresids != None and len(badresids)==0 and poorOnly == 0 and sconly != 1 :
            shutil.copyfile("pre."+str(i)+"."+rtkmodel, str(i)+"."+rtkmodel )
            print "Renaming 2 ", "pre."+str(i)+"."+rtkmodel, " to ",str(i)+"."+rtkmodel 

        elif  nb != 1 and sconly != 1 :
            shutil.copyfile("pre."+str(i)+"."+rtkmodel, str(i)+"."+rtkmodel )
            print "Renaming 3 ", "pre."+str(i)+"."+rtkmodel, " to ",str(i)+"."+rtkmodel 
            ...
            if ((len(badresids)==0 and poorOnly == 1 )  or (sconly == 1 and poorOnly == 1)):
            ...
            if len(badresids) == 0:
                ...
                scPrepC, useGivenRot, useDEE = prepareChain.PrepareChain("PRL"), userot , 1
                debug = 1 
                if debug == 1 :

                    scp = SCplacement("pre."+str(i)+"."+rtkmodel , scReduction, dir_xyzout+str(i)+"."+rtkmodel, "dotfile", useDEE, mtzfn, "FP", "FC", "PHIC", "2F1-F2", esmin, esmax, addsc, useGivenRot, badresids, scPrepC).run
        ...
        if mtzfn != None :
            ...
            sfcheck(dir_xyzout+"/phased."+str(i)+".mtz",dir_xyzout+"/"+str(i)+"."+rtkmodel,f1label,sigf1label,freeRlabel ) 

            if (os.path.isfile("sfcheck.log")==False) :
                print "no sfcheck file ";
                
            else :
                prefix = str(i)+"."+rtkmodel ; sfcheckfile1 = "sfcheck.%s.log"%prefix
            ...
            sfcheck(dir_xyzout+"/phased.pre."+str(i)+".mtz",dir_xyzout+"/pre."+str(i)+"."+rtkmodel,f1label,sigf1label,freeRlabel ) 


            if (os.path.isfile("sfcheck.log")==False) :
                print "no sfcheck file ";
            else :
                prefix = "pre." + str(i)+"."+rtkmodel; sfcheckfile2 = "sfcheck.%s.log"%prefix
