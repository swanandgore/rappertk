...
def locateRegionsRandomize3(resids, chids, badresids,modelN2C=1,buildType="notpoor") :
    ...
    for k in keys :
        ...
        if bad[k] and not loopOn :
            ...
            loopOn = 1
            loops.append( [k,k] )
            print k+1, len(keys), len(chids.keys())
            if k+1 not in chids.keys():
                print "DEal",k,resids[k],chids[k],k+1,resids[k+1]
...
def break_chain_in_bands( start, stop, pts, min_band_size,max_band_size,res,resns,resids):
    ...
    while (i <  length ) : 
        ...
        if (n_remaining == 0 and  n_total_remaining != 0) : 
            ...
            print "band_size",band_size,  min_band_size , max_band_size
...
class Multiloop :
    def __init__(s, pdbfile, badresids, mconly, caRad, scRad, scReduction, guidedSampling, popsize, backtrack, nmodels, outpdb, restrGen, prepC,sconly=None, cacaCutoff = 5.,null=[],modelN2C=0,natt=5,missingpts = {},loopclosure = 1 , allOpt=0, edOpt=0,ssfile=None):
    ...
    def run(s) :
        ...
        for wer in range(0,s.natt*20):
            print "rebanding noooooooooooooooooooowwwwwwwwwwwww"
...
#                        print nanchor,canchor,nanchor1,canchor1 
    ...
    ################# End - - - - Add missing atoms ###################################################                
            ...
            if s.ssfile !=None:
                ...
                from emrefine import makeHelix
                helices , sheets = [], [] 
                helices , sheets = parseSSfileAK(s.ssfile,resids,resnums)
            ...
            #mergeBRlists(blist, bfoll, numtrials, rlist, [], [], [], ssList, [], [])
            else :
                ssList = []
    ...
    #                print "makind fummy", resids[k],an
            ...
            for xr in ssList : extraRestraints.append(xr)
            rsize = len(extraRestraints)
            for ori in range(len(ssList)) :
            ...
            for xr in rlist : extraRestraints.append(xr)
            if s.allopt   == 1 :
                rsize = len(extraRestraints)
    ...
    #        for r in rlist :
            ...
            while (nbuilt < s.nmodels) :
                print "WE ARE The Champs At", nbuilt
                ...
                if len(unbuiltpts1) == 0 :
                    ...
                    break
            if nbuilt == s.nmodels :
                print "returning with 1 .. ", nbuilt, s.nmodels
                return nbuilt,all_built,all_unbuilt

        print "returning with  2  ..", nbuilt, s.nmodels
        return nbuilt,all_built,all_unbuilt
