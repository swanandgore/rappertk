...
def makeCrystLine(A,B,C,ALPHA,BETA,GAMMA, SG, Z=None) :
    from pdbinfo import a,b,c,alpha,beta,gamma, sg, z
    l = "CRYST1"
    l += "%9.3f" % A + "%9.3f" % B + "%9.3f" % C
    l += "%7.2f" % ALPHA + "%7.2f" % BETA + "%7.2f" % GAMMA
    l += "%10s" % SG
    if Z != None : l += "%4d" % Z
    return l + "\n"
...
def changeAtomcrds(line, newcrd) :
    from pdbinfo import xcrd, ycrd, zcrd
    if len(line) < zcrd[1] : line += (zcrd[1]+1 - len(line)) * ' '
    return line[0:xcrd[0]] + "%8.3f"%newcrd[0] + "%8.3f"%newcrd[1] + "%8.3f"%newcrd[2] + line[zcrd[1]:]
def changeAtomname(line, newaname) :
    from pdbinfo import atmn
    if len(line) < atmn[1] : line += (atmn[1]+1 - len(line)) * ' '
    return line[0:atmn[0]] + newaname + line[atmn[1]:]
def changeResn(line, newresn) :
    from pdbinfo import resn
    if len(line) < resn[1] : line += (resn[1]+1 - len(line)) * ' '
    return line[0:resn[0]] + newresn + line[resn[1]:]
def changeChid(line, newchid) :
    from pdbinfo import chid
    if len(line) < chid[1] : line += (chid[1]+1 - len(line)) * ' '
    return line[0:chid[0]] + newchid + line[chid[1]:]
def changeSegid(line, segid) :
    from pdbinfo import segi
    if len(line) < segi[1] : line += (segi[1]+1 - len(line)) * ' '
    if type(segid) == type(1) : return line[0:segi[0]] + "%4d" % segid + line[segi[1]:]
    return line[0:segi[0]] + segid + line[segi[1]:]
def changeOccupancy(line, occupancy) :
    from pdbinfo import occu
    if len(line) < occu[1] : line += (occu[1]+1 - len(line)) * ' '
    return line[0:occu[0]] + "%6.2f" % occupancy + line[occu[1]:]
def changeBfactor(line, bfactor) :
    from pdbinfo import bfac
    if len(line) < bfac[1] : line += (bfac[1]+1 - len(line)) * ' '
    return line[0:bfac[0]] + "%6.2f" % bfactor + line[bfac[1]:]
def changeBfactorOccupancy(line, bfactor, occupancy) :
    from pdbinfo import occu, bfac
    linetrail = ''
    if len(line) > 66 : linetrail = line[66:]
    line = line[0:occu[0]] + "%6.2f" % occupancy + "%6.2f" % bfactor + linetrail
    return line
def isPdbAtomLine(line) :
	return re.compile("^ATOM").search(line) or re.compile("^HETATM").search(line)

def isHetAtomLine(line) :
	return re.compile("^HETATM").search(line)
...
def isAAres(resn) :
	return getResidueCategory(resn) == ['aa']
...
def isPdbHetLine(line) :
	...
	return None
def makeModelLine(modname) :
    return "MODEL     %s" % modname
...
def line2inscode(line) :
    return line[ pdbinfo.ic[0]:pdbinfo.ic[1] ]
def line2bfac(line) :
    return string.atof( line[ pdbinfo.bfac[0]:pdbinfo.bfac[1] ] )
def line2occu(line) :
    return string.atof( line[ pdbinfo.occu[0]:pdbinfo.occu[1] ] )
def line2crdstr(line) :
	return ( line[pdbinfo.xcrd[0]:pdbinfo.xcrd[1]] + line[pdbinfo.ycrd[0]:pdbinfo.ycrd[1]] + line[pdbinfo.zcrd[0]:pdbinfo.zcrd[1]] )
def line2crd(line) :
...
def line2atomnumber(line) :
	return line[pdbinfo.atmi[0]:pdbinfo.atmi[1]]    
def line2atomid(line) :
	return line2resid(line) + line2atomname(line)
def line2segid(line) :
    if len(line) < pdbinfo.segi[1] : return None
    return line[pdbinfo.segi[0]:pdbinfo.segi[1]]
def line2resn(line) :
...
class protein :
	def __init__(self, filenameORlines, read_hydrogens, read_waters, read_hets, remove_duplicate_water=0, remove_duplicates=1, read_aa_only=0) :
		if 'list' in "%s" % type(filenameORlines) :
			self.name = "read_from_lines"
			#print "Reading protein from %d lines" % len(filenameORlines)
			self.readFromLines(filenameORlines, read_hydrogens, read_waters, read_hets, remove_duplicate_water, remove_duplicates, read_aa_only)
		else :
			self.name = filenameORlines
			f = open(filenameORlines, 'r')
			if not f : sys.exit(1)
                        fileList = f.readlines()
			filelines = f.readlines()
			f.close()
			self.readFromLines(fileList, read_hydrogens, read_waters, read_hets, remove_duplicate_water, remove_duplicates, read_aa_only)

	def readFromLines(self, filelines, read_hydrogens, read_waters, read_hets, remove_duplicate_water, remove_duplicates, read_aa_only) :
		...
		for l in filelines :
			...
			l = change_O_OW_HOH(l)
                        ...
                        if line2resn(l) in nonStdAA.keys() :
                            print 
                            print "***** WARNING!! Skipping residue [%s] *********"%line2resn(l)
                            print "Current version of Rappertk does not support non std amino acids and modified residues"
                            print "Check for regular updates on www-cryst.bioc.cam.ac.uk/members/anjum"
                            print 
                            continue                        
			...
			if not isPdbAtomLine(l) :
                            continue
			if line2resn(l) == "MSE" and l[0:6] == "HETATM" : l = "ATOM  " + l[6:]
			...
			if read_aa_only==1 and not isPdbAAline(l) :
                            continue
			...
			if remove_duplicates == 1 :
				...
				elif isPdbHOHline(l) and remove_duplicate_water == 0:
                                    atomlines.append(l)
		...
		if len(reslines) == 0 : reslines.append( (0, len(atomlines)) )
		else : reslines.append( (reslines[len(reslines)-1][1], len(atomlines)) )
