...
from loopbuildV2 import Multiloop, incompleteSCcorrection2
import prepareChain
...
def main() :
    ...
    if (os.path.isfile(pdbfilepath)==False) :
        ...
        pdbfilepath = options.dir_xyzout + "/modelinit.pdb"
...
###############################################################################################################################
    ...
    mapfilepath = None ; mtzfilepath = None 
    if  options.mapfn !=None:
        ...
        if (os.path.isfile(mapfilepath)==False) :
            ...
            shutil.copyfile(mapfilepath, "%s/init.map" % (options.dir_xyzout,options.mapfn))
            mapfilepath = options.dir_xyzout+"/init.map"
        ...
        if (os.path.isfile(mtzfilepath)==False) :
            ...
            mtzfilepath = options.dir_xyzout+"/init.mtz"
        ...
        print "ff",options.f1label,options.f2label,options.phiclabel,options.sigf1label
        ...
        fft(mtzfilepath, fcmap,  options.f1label,options.f2label,options.phiclabel,options.sigf1label,options.m,options.n,options.freerlabel)
        mtzfilepath = None
        mapfilepath = ccp4map
    ...
    multiPrepC = prepareChainV2.PrepareChain(options.rotLib)
    ...
    if mtzfilepath !=None or mapfilepath != None:
        if options.allOpt == 1:
            options.edOpt = 1
	...
	#    for sc in scmiss :
        ...
        #modelRenderer = ModelRenderer(res, resns, chids, resnums, inscodes, [],modelIn)
       	#modelRenderer.render(pts)
        #removeMODEL(modelIn) ;         adjustBfac2(modelIn, options.pdbfile,1, options.mcBfac, options.scBfac)
    if options.start == None and options.stop  == None and options.poorOnly == 0 :
        badresids = None
    ...
    for  i in range(options.nmodels):
        ...
        if options.opsax == 0 or mapfilepath ==None or options.mconly == 1 or (opsaxoff == 1 and nb!=1):
            ...
            if len(badresids) > 250:
                ...
                nb2 = SCplacement("pre"+rtkmodel+"."+str(i), options.scReduction, rtkmodel+"."+str(i) , "dotfile", useDEE, mapfilepath, options.f1label, options.f2label , options.phiclabel, mapcoeff, esmin, esmax, None, useGivenRot, badresids, scPrepC).run()
                if nb2 != 1:
