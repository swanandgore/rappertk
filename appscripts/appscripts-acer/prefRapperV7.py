...
def main() :
    ...
    parser.add_option("--dir-xyzin", action='store', type='string', dest='dir_pdbfile', help='Name of coordinate input structure')
    ...
    parser.add_option("--mconly", action='store', type='int', dest='mconly', help='Build mainchain only', default=0)
    ...
    parser.add_option("--models-get-native-bfactors", action='store', type='int', dest='nativeBfac', help='[0/1] 1:Yes 0:No. Assign B-factors of remodelled atoms to original values', default=0)
    ...
    parser.add_option("--modelN2C", action='store', type='int', dest='modelN2C', help='Model N2C fragment  Yes:1, No:0', default=0)
    ...
    print "	--framework-ca-threshold ".ljust(50), 
    print "%20s "%options.fcarad 
    print "	--framework-sc-threshold ".ljust(50), 
    print "%20s "%options.fscrad
    ...
    print "	--build-loop ".ljust(50), 
    print "%20s "%options.build_loop 
    ...
    print "	--framework ".ljust(50), 
    print "%20s "%options.framework 
...
#    fcmap = "fft.fc.ext.map"
    ...
    for  i in range(options.nmodels):
        ...
        #print "HERE I AM",options.sconly 
        #import sys ; sys.exit()

        #if (options.sconly == 0 and ( badresids == None or len(badresids) != 0 or options.poorOnly != 0)):
        print
        print
        ...
        if ( ( badresids == None or len(badresids) != 0 or options.poorOnly == 1)): 
            ...
            ml = Multiloop(modelIn, badresids, options.mconly, options.caRad, options.scRad, scvdwr, guidedsampling, popsize,
                           options.backtrack, 1 , "pre."+str(i)+"."+rtkmodel, xrayRestGen, multiPrepC,options.sconly, options.cacaCutoff,nullres,options.modelN2C,options.natt,missingpts) 
        ...
        if options.opsax == 0 or options.mapfn==None or options.mconly == 1 or opsaxoff == 1:
            ...
            if options.poorOnly == 1 and options.sconly == 0 : 
                ...
                badresids = list(  set(  findAllSC( "pre."+str(i)+"."+rtkmodel)  )  )


            else :
                print "Error 502.Please report problem to ak459@cam.ac.uk"
                import sys ; sys.exit()
        ...
        if options.mtzfn != None or options.mapfn != None :
            ...
            sfcheck("phased."+str(i)+".mtz",str(i)+"."+rtkmodel,options.f1label,options.sigf1label,options.freeRlabel ) ## automatic water addition can go here
...
## automatic water addition can go here
