...
def findChis(res, pts, ind, resname) :
    if not resname in chiRes.keys() : return []
    chis = []
...
def comparePhiPsiOmegaChi(refprot, testprot, mconly) :
    ...
    for ind,rid in resids.items() :
        if not rid in resid2ind.keys() : continue
        ...
        if not mconly :
            ...
            if len(chis) > 0 :
                ...
                chi1diffcnt += 1
                if math.fabs( dihedDiff(chis[0],chisTest[0]) ) < 40 : chi1accu += 1
            if len(chis) > 1 :
