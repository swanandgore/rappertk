...
modulesInUse = ['builders', 'restraints', 'samplers', 'geometry', 'misc',]
...
args = ['rappertk'] + [os.environ["RTKROOT"] + os.sep + "appscripts" + os.sep + sys.argv[1] + ".py"] + sys.argv[2:] #padding 0th arg

print "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"
print "  Welcome to RapperTK - a versatile engine for conformational sampling of macromolecules   "
print "        Developed by Swanand Gore at Dept of Biochemistry, University of Cambridge         "
print "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"
